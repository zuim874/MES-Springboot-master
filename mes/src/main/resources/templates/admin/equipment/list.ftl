...
<script>
    layui.use(['form', 'table', 'spLayer', 'spTable'], function () {
        ...
        var tableIns = spTable.render({
            url: '${request.contextPath}/admin/equipment/page',
            cols: [
                [{
                    type: 'checkbox'
                }, {
                    field: 'code', title: '设备编号', width: 150
                }, {
                    field: 'name', title: '设备名称', width: 150
                }, {
                    field: 'modelSpec', title: '型号与规格', width: 150
                }, {
                    field: 'connectionType', title: '连接方式', width: 180
                }, {
                    field: 'status', title: '状态', width: 90, templet: function (d) {
                        return d.status === '1' ? '作业中' : '空闲';
                    }
                }, {
                    ...
                    fixed: 'right', field: 'operate', title: '操作', toolbar: '#js-record-table-toolbar-right', unresize: true, width: 180
                }]
            ]
        });
        ...
        table.on('toolbar(js-record-table-filter)', function (obj) {
            if (obj.event === 'add') {
                spLayer.open({
                    title: '添加设备',
                    area: ['90%', '90%'],
                    content: '${request.contextPath}/admin/equipment/add-or-update-ui'
                });
            }
        });
        ...
        table.on('tool(js-record-table-filter)', function (obj) {
            ...
            if (obj.event === 'delete') {
                layer.confirm('确认删除吗？', function (index) {
                    $.ajax({
                        url: '${request.contextPath}/admin/equipment/delete',
                        type: 'POST',
                        data: { id: data.id },
