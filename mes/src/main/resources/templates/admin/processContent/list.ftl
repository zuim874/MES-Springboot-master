...
<div class="splayui-container">
    <div class="splayui-main">
        ...
        <table class="layui-hide" id="js-content-table" lay-filter="js-content-table-filter"></table>
    </div>
</div>
...
<script type="text/html" id="js-content-table-toolbar-right">
    <a class="layui-btn layui-btn-xs layui-btn-normal" lay-event="detail"><i class="layui-icon layui-icon-senior"></i>编制工艺内容</a>
</script>
...
<script>
    layui.use(['form', 'table', 'spLayer', 'spTable'], function () {
        ...
        var contentTableIns = spTable.render({
            url: '${request.contextPath}/admin/processContent/page',
            elem: '#js-content-table',
            cols: [[
                ...
                {fixed: 'right', title: '操作', toolbar: '#js-content-table-toolbar-right', width: 140}
            ]]
        });

        form.on('submit(js-search-filter)', function (data) {
            contentTableIns.reload({where: data.field});
            return false;
        });

        table.on('tool(js-content-table-filter)', function (obj) {
            var data = obj.data;
            if (obj.event === 'detail') {
                spLayer.open({
                    title: '工艺内容编制 - ' + data.name,
                    area: ['960px', '620px'],
                    content: '${request.contextPath}/admin/processContent/detail-ui',
